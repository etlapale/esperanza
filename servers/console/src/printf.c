...
/* Video address */
#define VIDEO (0xb8b8000 + COLUMNS * (25 - LINES) * 2)
...
init_printf (void)
{
  ...
  console_xpos = console_ypos = 0;
  //  console_ypos = 2;
  console_video = (unsigned char *) VIDEO;

  for (i = 0; i < COLUMNS * LINES * 2; i++)
    *(console_video + i) = 0x00;
}
...
print_char (int c)
{
  ...
  *(console_video + (console_xpos + console_ypos * COLUMNS) * 2 + 1)
    = ATTRIBUTE;
